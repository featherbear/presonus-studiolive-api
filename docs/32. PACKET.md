...
> **Note**  
`Payload Size = length(Payload Type) + length(Payload Data)`
...
> i.e If a message is sent with `C-Byte A = j`, `C-Byte B = e` a response will be sent containing `C-Byte A = e`, `C-Byte B = j` so that the response can be matched with the request.
...
The console will then reply with a response JSON payload  
`{"id": "SubscriptionReply"}`  
and additionally a ZLIB payload
