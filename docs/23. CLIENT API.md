...
`<Client>.setMute(selector: ChannelSelector, status: boolean | 'toggle')`  
Set the mute status of a channel

`<Client>.getSolo(selector: ChannelSelector)`  
Get solo status of a channel

`<Client>.toggleSolo(selector: ChannelSelector)`  
Toggle the solo status of a channel

`<Client>.setSolo(selector: ChannelSelector, status: boolean | 'toggle')`  
