...
client.on(MESSAGETYPES.ZLIB, function (ZB: ZlibPayload) {
    ...
    let floor = 0x3a970133
    const { chnum, name, username, color, select, solo, volume, mute, pan } = ZB.children.line.children.ch1.values
    console.log({ chnum, name, username, color, select, solo, volume, volume2: intToLE(volume), volume3:
        
        intToLE(volume).readInt32LE() === 0 ? 0 : (intToLE(volume).readInt32LE() - floor) / (0X3f800000 - floor), mute, pan });
