  ...
  _handleRecvPacket (packet) {
    ...
    switch (messageCode) {
      ...
      case MessageTypes.Setting:
        let idx = data.indexOf(0x00) // Find the NULL terminator of the key string
        if (idx != -1) {
          let key = data.slice(0, idx).toString()

          // Most setting packets are `key\x00\x00\x00...`
          // but some (i.e. filter groups) have `key\x00\x00\x01`
          let partA = data.slice(idx + 1, idx + 3 /* 1+2 */)
          let partB = data.slice(idx + 3)
          data = {
            name: key,
