...
export function zlibParseNode(node: ZlibInputNode, { base = {} }: zlibNodeParserOptArgs = {}): ZlibNode {
	...
	const keyHandlers: { [k in keyof ZlibInputNode]: (data) => void } & {
		[k: string]: ((data) => void) | null;
	} = {
		children(data) {
...
export function dumpNode(node: ZlibNode, recurseDepth?: number) {
	...
	if ((typeof recurseDepth === "undefined" || recurseDepth > 0) && Object.keys(node).length > 0) {
		data.children = Object.entries(node).reduce(
			(obj, [key, node]) => ({
				...obj,
				[key]: dumpNode(<ZlibNode>node, recurseDepth ? recurseDepth - 1 : recurseDepth),
			}),
			{},
		);
	}
